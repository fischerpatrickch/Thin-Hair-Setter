...
var About = group6.add("statictext", undefined, undefined, {name: "About"}); 
    About.helpTip = "For best results, run Thin Hair Setter before applying local character formatting (e.g. bold, italic, size changes). Thin Hair Setter automates detailed microtypographic adjustments in InDesign — spacing, punctuation, numerals and more. Built by Patrick Fischer at IDCE, HGK Basel FHNW, as part of the Generative Toolmaking module under the supervision of Ted Davis. Visit my website: fischerpatrick.ch";
    About.text = "?"; 
...
if (result === 1) {
   ...
   if (Selection.selection.index === 0) { 								// All Pages
    ...
    // PointSize nur aktuelle Seite
    for (var p = 0; p < spreadPages.length; p++) {
        var pageFrames = spreadPages.item(p).textFrames;
        for (var i = 0; i < pageFrames.length; i++) {
            app.findGrepPreferences = NothingEnum.nothing;
            app.findGrepPreferences.findWhat = '\\d+';
            var found = pageFrames[i].findGrep();
            for (var j = found.length - 1; j >= 0; j--) {
                var currentSize = found[j].pointSize;
                typo(found[j], 'pointSize', currentSize - edittext1.text);
            }
            app.findGrepPreferences = NothingEnum.nothing;
        }
    }
    ...
    // PointSize nur Selektion
    for (var i = 0; i < savedSelection.length; i++) {
        app.findGrepPreferences = NothingEnum.nothing;
        app.findGrepPreferences.findWhat = '\\d+';
        var found = savedSelection[i].findGrep();
        for (var j = found.length - 1; j >= 0; j--) {
            var currentSize = found[j].pointSize;
            typo(found[j], 'pointSize', currentSize - edittext1.text);
        }
...
function doMicroType(sel){
  ...
  var weekdays = [
	...
	//Phone Numbers
	 ...
	 var phonePattern4 = new RegExp('(\\+)(\\d{2})(\\(0\\))(\\d{2})(\\d{3})(\\d{2})(\\d{2})', 'g')
	 sel.contents = sel.contents.replace(phonePattern4, '$1$2 $3$4 $5 $6 $7')

	  var phonePattern4_2 = new RegExp('(\\+)(\\d{2}) (\\(0\\))(\\d{2}) (\\d{3}) (\\d{2}) (\\d{2})', 'g')
	 sel.contents = sel.contents.replace(phonePattern4_2, '$1$2 $3$4 $5 $6 $7')

     var phonePattern4_3 = new RegExp('(\\+)(\\d{2}) (\\(0\\)) (\\d{2})(\\d{3}) (\\d{2}) (\\d{2})', 'g');
sel.contents = sel.contents.replace(phonePattern4_3, '$1$2 $3$4 $5 $6 $7');


	  var phonePattern3 = new RegExp('(\\+)(\\d{2})(\\d{2})(\\d{3})(\\d{2})(\\d{2})', 'g')
	 sel.contents = sel.contents.replace(phonePattern3, '$1$2 $3 $4 $5 $6')

	   var phonePattern3_2 = new RegExp('(\\+)(\\d{2}) (\\d{2}) (\\d{3}) (\\d{2}) (\\d{2})', 'g')
	 sel.contents = sel.contents.replace(phonePattern3_2, '$1$2 $3 $4 $5 $6')

	   var phonePattern2 = new RegExp('(0\\d{3})(\\d{2})(\\d{3})(\\d{2})(\\d{2})', 'g')
	 sel.contents = sel.contents.replace(phonePattern2, '$1 $2 $3 $4 $5')

	  var phonePattern2_2 = new RegExp('(0\\d{3}) (\\d{2}) (\\d{3}) (\\d{2}) (\\d{2})', 'g')
	 sel.contents = sel.contents.replace(phonePattern2_2, '$1 $2 $3 $4 $5')

	 var phonePattern1 = new RegExp('(0\\d{2})(\\d{3})(\\d{2})(\\d{2})', 'g')
	 sel.contents = sel.contents.replace(phonePattern1, '$1 $2 $3 $4')

	var phonePattern1_2 = new RegExp('(0\\d{2}) (\\d{3}) (\\d{2}) (\\d{2})', 'g')
  ...
  sel.contents = sel.contents.replace(dashTime, '$1 – $3');
    ...
    var threeDots = new RegExp('\\.\\.\\.', 'g');
    sel.contents = sel.contents.replace(threeDots, '…');
	...
	if(Ident.value == true) {
	 ...
	 var dateWord = new RegExp('(\\d+)\\.([a-zäöüA-ZÄÖÜ]+)', 'g');
	 sel.contents = sel.contents.replace(dateWord, '$1. $2');

	  var dateWordSpace = new RegExp('(\\d+)\\.([a-zäöüA-ZÄÖÜ]+)', 'g');
