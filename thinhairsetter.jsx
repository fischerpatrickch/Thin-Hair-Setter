...
var About = group6.add("statictext", undefined, undefined, {name: "About"}); 
    About.helpTip = "Micro Hair Setter automates detailed microtypographic adjustments in InDesign — spacing, punctuation, numerals and more. Built by Patrick Fischer at IDCE, HGK Basel FHNW, as part of the Generative Toolmaking module under the supervision of Ted Davis. Visit my website: fischerpatrick.ch";
    About.text = "?"; 
  ...
  var weekdays = [
	...
	if(Units.value == true){
	 ...
	 sel.contents = sel.contents.replace(centimeter, '$1 $2');
	    ...
	    var meterSpace = new RegExp('(\\d+) (\\m)', 'g');
	 sel.contents = sel.contents.replace(meterSpace, '$1 $2');

	  var meter = new RegExp('(\\d+)(\\m)', 'g');
	 ...
	 sel.contents = sel.contents.replace(kilometer, '$1 $2');
	  ...
	  var grammSpace = new RegExp('(\\d+) (\\g)', 'g');
	 sel.contents = sel.contents.replace(grammSpace, '$1 $2');

	  var gramm = new RegExp('(\\d+)(\\g)', 'g');
	...
	var literSpace = new RegExp('(\\d+) (\\l)', 'g');
	 sel.contents = sel.contents.replace(literSpace, '$1 $2');

	  var liter = new RegExp('(\\d+)(\\l)', 'g');
